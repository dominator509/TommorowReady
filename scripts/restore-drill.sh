...
artifact=.agent/state/backups/tomorrowready-local.dump.enc
[ -s "$artifact" ] || { echo "restore drill: FAIL - backup missing" >&2; exit 1; }
plaintext=.agent/state/backups/tomorrowready-restore.dump.tmp
...
cleanup() { rm -f "$plaintext"; db_admin -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS $restore_db WITH (FORCE)" >/dev/null; }
trap cleanup EXIT
cleanup
pnpm exec tsx scripts/backup-crypto.ts decrypt "$artifact" "$plaintext"
db_admin -v ON_ERROR_STOP=1 -c "CREATE DATABASE $restore_db" >/dev/null
docker compose exec -T postgres sh -eu -c 'exec pg_restore --username="$POSTGRES_USER" --dbname="$1" --no-owner --no-acl' sh "$restore_db" <"$plaintext"
