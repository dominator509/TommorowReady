...
Run `sh scripts/bootstrap-local-env.sh`, then `docker compose up -d --wait`. The generated `.env` is ignored, mode-restricted where supported, and contains unique local-only PostgreSQL, S3, session, and field-encryption credentials. `POSTGRES_PASSWORD` is infrastructure-only and is not an application runtime variable. Mailpit captures real SMTP traffic locally. Optional external providers remain unset and fail closed.
